...
# Regenerate the high-level package graph (one node per workspace
# package, edges between them). Output is a self-contained mermaid block
# that GitHub renders inline in markdown — no graphviz dependency.
depcruise-graph: install
    ...
    {{ nix_shell }} ./node_modules/.bin/depcruise \
        ...
        --no-progress -T mermaid packages > "$tmp"
    {
        echo '<!-- Generated by `just depcruise-graph`. Do not edit by hand. -->'
        echo
        echo '# Package dependency graph'
        echo
        echo 'High-level view of workspace imports — one node per package, collapsed from `.dependency-cruiser.mjs`. Re-run `just depcruise-graph` after touching package boundaries.'
        echo
        echo '```mermaid'
        cat "$tmp"
        echo '```'
