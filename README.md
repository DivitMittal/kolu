...
- Attention pings — when an agent is waiting on you (or has finished with an unread completion), an alert dot pulses on the workspace-switcher pill, the canvas tile's title bar, and the minimap rectangle, so you can spot it from any zoom level without panning. <kbd>Ctrl+Tab</kbd> (or <kbd>Alt+Tab</kbd>) cycles terminals in MRU order: hold the modifier, press Tab to advance, release to commit
...
- **Border encodes agent state** — workspace-switcher pills, expanded cards, canvas tiles, and minimap rectangles share one visual vocabulary: a static accent ring while the agent is `thinking`/`tool_use`, a breathing alert pulse while `waiting`, and an inset focus glow when the tile is the active terminal. The two channels coexist — outer ring carries bucket, inset glow carries identity — so a focused awaiting terminal still reads at a glance from anywhere on the canvas
