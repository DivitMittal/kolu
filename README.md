...
| `packages/pty-host/`                 | The local PTY-host daemon's **entire closure** — the multi-client PTY-owner primitive ([node-pty](https://github.com/microsoft/node-pty) + [@xterm/headless](https://www.npmjs.com/package/@xterm/headless) screen mirror + VT-derived taps: OSC 7 cwd, OSC 0/2 title, OSC 633 command-run, exit, foreground, each fanned out to many consumers via a bounded `Channel`; race-free `attach`) **plus** the daemon's wire contract (`ptyHostSurface`), build identity (`buildId`), startup utils, and the runnable `kolu --stdio` entrypoint (`./daemon` subpath). Keeping the whole survivor in one package means `KOLU_PTY_HOST_BUILD_ID` (hashed from this dir) covers everything that defines the daemon, so a wire-shape change flips the staleness key. Since [#951 R4c](docs/plans/remote-terminals.html) it runs as a long-lived `kolu --stdio` daemon that **survives a kolu-server restart** (so terminals persist across a deploy, replacing tmux/zmx — [#671](https://github.com/juspay/kolu/issues/671)); kolu-server is its socket client |
