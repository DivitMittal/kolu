...
| `packages/integrations/claude-code/` | Claude Code detection — JSONL transcript tailing + Claude Agent SDK; exports a `claudeCodeProvider` `AgentProvider`. The phantom-pill decay's process-subtree probe spawns `ps` via the Nix-pinned `KOLU_PS_BIN` (procps on Linux, system `/bin/ps` on Darwin), never the runtime PATH |
