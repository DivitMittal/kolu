...
| `packages/integrations/git/`         | Pure git operations — repo resolution, worktree lifecycle, diff review, path security; every `git` / `fs` call routes through `kolu-io`'s `Executor` so the same code drives local and SSH-routed remote repos; schemas re-exported by `kolu-common`                                                          |
...
| `packages/integrations/io/`          | Filesystem & I/O primitives — the `Executor` interface (`exec` / `readFile` / `statMtimeMs` / `watch` / `queryDb`) + default `localExecutor`; the `Host` type (`extends Executor`) declares PTY spawning + identity for remote backends; refcounted shared `fs.watch` (`createDirFilenameWatcher`) lives here |
