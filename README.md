...
The high-level dependency graph between these packages is regenerated by `just depcruise-graph` into [`docs/dependency-graph.md`](docs/dependency-graph.md). `just depcruise` (also run in CI as `ci/depcruise`) validates the import graph against layer rules in [`.dependency-cruiser.mjs`](.dependency-cruiser.mjs) — e.g. the client must not pull in integration runtime modules (allowed: `/schemas` subpaths + the `anyagent` contract package), integration packages must not depend on each other, and `kolu-transcript-core` must not depend on `kolu-transcript-html`. New rules start at `warn` severity; once their current findings are addressed, individual rules ratchet to `error` so regressions block CI.
