...
| `packages/server/`                   | [Hono](https://hono.dev/) kolu-server; also the `kolu --stdio` daemon (`agent/main.ts`) that hosts `@kolu/pty-host` + the provider DAG. kolu-server connects to the daemon over a unix socket and consumes its `agentMetadata` stream — local terminals reattach across a server restart |
| `packages/pty-host/`                 | The multi-client PTY-owner primitive — [node-pty](https://github.com/microsoft/node-pty) + [@xterm/headless](https://www.npmjs.com/package/@xterm/headless) screen mirror + VT-derived taps (OSC 7 cwd, OSC 0/2 title, OSC 633 command-run, exit, foregroundPid), each fanned out to many consumers via a bounded `Channel`. Owns *only* the PTY; race-free `attach` (snapshot+deltas). Consumed by the `kolu --stdio` daemon (`packages/server/src/agent/main.ts`), which serves it over a unix socket to kolu-server |
...
flowchart TB
  ...
  subgraph Server["kolu-server (Hono)"]
    Backend["LocalTerminalBackend\nunix-socket client · terminalMetadata · activity · sessions"]:::server
  end

  subgraph Agent["Agent — kolu --stdio daemon (own process; ssh-remote later)"]
  ...
  Xterm -->|"sendInput (WS)"| Backend
  Backend -->|"terminal.write\n(unix socket)"| Host
  Host -->|"attach stream\nsnapshot + deltas (unix socket)"| Backend
  Backend -->|"attach stream (WS)"| Xterm

  %% Metadata loop
  Host -.->|"OSC taps\n(7 cwd · 2 title · 633 cmd)"| Providers
  Providers -.->|"agentMetadata stream\n(events, unix socket)"| Backend
