/** Per-terminal provider DAG, extracted from `local.ts` so both
 *  `LocalTerminalBackend` (kolu-server's in-process backend) and the
 *  `kolu --stdio` agent (which runs the SAME providers on the remote
 *  host so detection sees the real filesystem / processes / sqlite)
 ...
 *
 *  Both consumers pass a `ProviderHooks` object so the providers
 *  don't need to know which backend they're running inside —
 *  metadata writes funnel through `hooks.update*Metadata`, recent
 *  repos/agents notifications only ride parent-side via optional
 *  callbacks.
 *
 *  Note on `git` channel: the GitHub PR provider chains off the
 *  `git` channel that the git provider publishes — so the channel
 *  has to be provided by the host (terminalChannels on parent,
 *  agent-local in-memory channel on agent).
 */
