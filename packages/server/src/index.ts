 ...
 * other invocation is the HTTP/WS **kolu-server** (`./server.ts`). The
 * daemon serves `agentSurface` over a unix socket (one per `$KOLU_STATE_DIR`);
 * kolu-server's supervisor spawns this same binary with `--stdio` +
 * `detached: true` so the daemon survives a kolu-server restart and its PTYs
 * reattach.
 *
