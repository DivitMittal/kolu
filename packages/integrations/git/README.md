...
| `resolve.ts`   | `resolveGitInfo`, `watchGitHead`, `gitInfoEqual`, `hasGitDir`, `subscribeGitInfo` | Repo context resolution + git metadata watching + combined subscribe loop |
...
1. Calls `subscribeGitInfo(cwd, onChange)` — the integration owns the resolve + `.git/HEAD`/config watch + re-resolve loop, including dedup via `gitInfoEqual` and `git init` detection (same-cwd `setCwd` on a not-yet-a-repo checks `.git` and re-resolves if it appeared)
