...
| `resolve.ts`   | `resolveGitInfo`, `watchGitHead`, `gitInfoEqual`, `hasGitDir`, `subscribeGitInfo` | Repo context resolution + `.git` entry / `.git/HEAD` watching + combined subscribe loop |
...
1. Calls `subscribeGitInfo(cwd, onChange)` — the integration owns the resolve + `.git` entry / `.git/HEAD` watch + re-resolve loop, including dedup via `gitInfoEqual` and `git init` detection while a terminal is already sitting in a non-repo directory
