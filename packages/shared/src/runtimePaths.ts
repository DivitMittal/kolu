 ...
 * connects to.
 *
 * Lives in `kolu-shared` (zero deps) so BOTH sides of the socket can compute
 * the same paths without a package cycle: the daemon entrypoint
 * (`@kolu/pty-host`'s `daemonMain.ts`) binds the socket here, and kolu-server's
 * supervisor connects to it. Keeping it in `packages/server/` would force the
 * daemon — which must NOT depend on kolu-server — to import server code.
 *
 * The daemon socket lives at a *fixed* path so a *restarted* kolu-server finds
 * the *same* running daemon — that stable key is `$KOLU_STATE_DIR`, the
 * directory kolu already uses for sessions / preferences / run-id. R4c adds
 * `pty-host.{pid,sock,log}` alongside.
 *
 * `$KOLU_STATE_DIR` is the dev/prod multiplexer: `just dev` uses
 * `./.kolu-dev`, the production install uses `~/.config/kolu`, the test
