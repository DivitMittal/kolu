...
factoring itself is wrong — and the factoring is the real
structure underneath both projections, the code's analogue of
that frame-invariant interval. Get it wrong and the defect shows
up in both at once: spatially (right now, in the propagation
...
to-one by single-axis ones across three review passes — and the few that surfaced
clustered in the later passes, after the revisions went in. Most findings, including the two centerpieces of this
